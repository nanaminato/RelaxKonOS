...
{
    private static readonly string[] LinuxSensitiveRoots = ["/", "/etc", "/root", "/proc", "/sys", "/dev"];
    public static string? ValidateShare(UpsertFileShareRequest request, bool windows)
    {
        ...
        if (windows ? IsUnsafeWindowsPath(full) || !IsWithin(full, @"D:\\RelaxKonOSShares") : IsUnsafeLinuxPath(full) || !IsWithin(full, "/srv/relaxkonos-shares")) return FileServiceProblemCodes.ConfigurationInvalid;
        if (request.GuestAllowed && !request.ReadOnly) return FileServiceProblemCodes.ConfigurationInvalid;
        if (request.Permissions.Count > 128 || request.Permissions.Any(p => !Principal().IsMatch(p.Principal) || IsInjection(p.Principal))) return FileServiceProblemCodes.ConfigurationInvalid;
    ...
    private static bool IsUnsafeLinuxPath(string full) => LinuxSensitiveRoots.Any(root => string.Equals(full, root, StringComparison.Ordinal));
    private static bool IsUnsafeWindowsPath(string full) => Path.GetPathRoot(full) == full || full.StartsWith(Environment.GetFolderPath(Environment.SpecialFolder.Windows), StringComparison.OrdinalIgnoreCase)
        || full.StartsWith(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles), StringComparison.OrdinalIgnoreCase)
        || full.StartsWith(Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData), StringComparison.OrdinalIgnoreCase);
    private static bool IsWithin(string path, string root) => string.Equals(path, root, OperatingSystem.IsWindows() ? StringComparison.OrdinalIgnoreCase : StringComparison.Ordinal)
        || path.StartsWith(root.TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar) + Path.DirectorySeparatorChar, OperatingSystem.IsWindows() ? StringComparison.OrdinalIgnoreCase : StringComparison.Ordinal);
    private static bool IsReparsePath(string full)
    {
        var root = Path.GetPathRoot(full)!;
        for (var part = root; ;)
        {
            if (Directory.Exists(part) && File.GetAttributes(part).HasFlag(FileAttributes.ReparsePoint)) return true;
            var next = Path.GetRelativePath(part, full).Split(Path.DirectorySeparatorChar, StringSplitOptions.RemoveEmptyEntries).FirstOrDefault();
            if (next is null) return false;
            part = Path.Combine(part, next);
        }
    }
