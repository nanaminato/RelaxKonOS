...
public static class GitEndpoints
{
    ...
    public static IEndpointRouteBuilder MapGitEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapPost("/repositories", async (GitRepositoryRegistration registration, ClaimsPrincipal principal, RelaxKonOS.Server.Git.IGitRepositoryService service, CancellationToken ct) =>
        {
            try
            {
        ...
        group.MapGet("/repositories/{id}", async (string id, ClaimsPrincipal principal, RelaxKonOS.Server.Git.IGitRepositoryService service, CancellationToken ct) =>
        {
        ...
        group.MapDelete("/repositories/{id}", async (string id, ClaimsPrincipal principal, RelaxKonOS.Server.Git.IGitRepositoryService service, CancellationToken ct) =>
        {
            if (!Guid.TryParse(id, out var repoId)) return Results.BadRequest();
        ...
        group.MapGet("/repositories/{id}/status", async (string id, ClaimsPrincipal principal, RelaxKonOS.Server.Git.IGitRepositoryService service, CancellationToken ct) =>
        {
            if (!Guid.TryParse(id, out var repoId)) return Results.BadRequest();
        ...
        group.MapGet("/repositories/{id}/branches", async (string id, ClaimsPrincipal principal, RelaxKonOS.Server.Git.IGitRepositoryService service, CancellationToken ct) =>
        {
            if (!Guid.TryParse(id, out var repoId)) return Results.BadRequest();
        ...
        group.MapGet("/repositories/{id}/branches/{name}/comparison", async (string id, string name, ClaimsPrincipal principal, RelaxKonOS.Server.Git.IGitRepositoryService service, CancellationToken ct) =>
        {
            if (!Guid.TryParse(id, out var repoId)) return Results.BadRequest();
        ...
        group.MapPost("/repositories/{id}/branches", async (string id, GitBranchCreateRequest request, ClaimsPrincipal principal, RelaxKonOS.Server.Git.IGitRepositoryService service, CancellationToken ct) =>
        {
            if (!Guid.TryParse(id, out var repoId)) return Results.BadRequest();
            return Results.Ok(await service.CreateBranchAsync(repoId, GetUserId(principal), request, ct));
        });

        group.MapDelete("/repositories/{id}/branches/{name}", async (string id, string name, ClaimsPrincipal principal, RelaxKonOS.Server.Git.IGitRepositoryService service, CancellationToken ct) =>
        {
            if (!Guid.TryParse(id, out var repoId)) return Results.BadRequest();
            return Results.Ok(await service.DeleteBranchAsync(repoId, GetUserId(principal), name, ct));
        });

        group.MapPost("/repositories/{id}/branches/{name}/rename", async (string id, string name, GitBranchRenameRequest request, ClaimsPrincipal principal, RelaxKonOS.Server.Git.IGitRepositoryService service, CancellationToken ct) =>
        {
            if (!Guid.TryParse(id, out var repoId)) return Results.BadRequest();
            return Results.Ok(await service.RenameBranchAsync(repoId, GetUserId(principal), name, request, ct));
        });

        group.MapPut("/repositories/{id}/branches/{name}/tracking", async (string id, string name, GitBranchTrackingRequest request, ClaimsPrincipal principal, RelaxKonOS.Server.Git.IGitRepositoryService service, CancellationToken ct) =>
        {
            if (!Guid.TryParse(id, out var repoId)) return Results.BadRequest();
            return Results.Ok(await service.SetBranchTrackingAsync(repoId, GetUserId(principal), name, request, ct));
        });

        group.MapPost("/repositories/{id}/checkout", async (string id, GitCheckoutRequest request, ClaimsPrincipal principal, RelaxKonOS.Server.Git.IGitRepositoryService service, CancellationToken ct) =>
        {
            if (!Guid.TryParse(id, out var repoId)) return Results.BadRequest();
            return Results.Ok(await service.CheckoutAsync(repoId, GetUserId(principal), request, ct));
        });

        group.MapPost("/repositories/{id}/commit", async (string id, GitCommitRequest request, ClaimsPrincipal principal, RelaxKonOS.Server.Git.IGitRepositoryService service, CancellationToken ct) =>
        {
            if (!Guid.TryParse(id, out var repoId)) return Results.BadRequest();
            return Results.Ok(await service.CommitAsync(repoId, GetUserId(principal), request, ct));
        });

        group.MapPost("/repositories/{id}/merge", async (string id, GitMergeRequest request, ClaimsPrincipal principal, RelaxKonOS.Server.Git.IGitRepositoryService service, CancellationToken ct) =>
        {
            if (!Guid.TryParse(id, out var repoId)) return Results.BadRequest();
            return Results.Ok(await service.MergeBranchAsync(repoId, GetUserId(principal), request, ct));
        });

        group.MapPost("/repositories/{id}/fetch", async (string id, ClaimsPrincipal principal, RelaxKonOS.Server.Git.IGitRepositoryService service, CancellationToken ct) =>
        {
            if (!Guid.TryParse(id, out var repoId)) return Results.BadRequest();
            return Results.Ok(await service.FetchAsync(repoId, GetUserId(principal), ct));
        });

        group.MapPost("/repositories/{id}/pull", async (string id, GitPullRequest request, ClaimsPrincipal principal, RelaxKonOS.Server.Git.IGitRepositoryService service, CancellationToken ct) =>
        {
            if (!Guid.TryParse(id, out var repoId)) return Results.BadRequest();
            return Results.Ok(await service.PullAsync(repoId, GetUserId(principal), request, ct));
        });

        group.MapPost("/repositories/{id}/push", async (string id, GitPushRequest? request, ClaimsPrincipal principal, RelaxKonOS.Server.Git.IGitRepositoryService service, CancellationToken ct) =>
        {
            if (!Guid.TryParse(id, out var repoId)) return Results.BadRequest();
        ...
        group.MapGet("/repositories/{id}/log", async (string id, int? limit, int? skip, string? reference, string? search,
            ...
            ClaimsPrincipal principal, RelaxKonOS.Server.Git.IGitRepositoryService service, CancellationToken ct) =>
        {
            if (!Guid.TryParse(id, out var repoId)) return Results.BadRequest();
        ...
        group.MapGet("/repositories/{id}/commits/{sha}", async (string id, string sha, ClaimsPrincipal principal, RelaxKonOS.Server.Git.IGitRepositoryService service, CancellationToken ct) =>
        {
            if (!Guid.TryParse(id, out var repoId)) return Results.BadRequest();
        ...
        group.MapGet("/repositories/{id}/diff", async (string id, string path, bool? staged, string? @ref, ClaimsPrincipal principal, RelaxKonOS.Server.Git.IGitRepositoryService service, CancellationToken ct) =>
        {
            if (!Guid.TryParse(id, out var repoId)) return Results.BadRequest();
        ...
        group.MapPost("/repositories/{id}/reset", async (string id, GitResetRequest request, ClaimsPrincipal principal, RelaxKonOS.Server.Git.IGitRepositoryService service, CancellationToken ct) =>
        {
            if (!Guid.TryParse(id, out var repoId)) return Results.BadRequest();
            return Results.Ok(await service.ResetAsync(repoId, GetUserId(principal), request, ct));
        });

        group.MapPost("/repositories/{id}/restore", async (string id, GitRestoreRequest request, ClaimsPrincipal principal, RelaxKonOS.Server.Git.IGitRepositoryService service, CancellationToken ct) =>
        {
            if (!Guid.TryParse(id, out var repoId)) return Results.BadRequest();
            return Results.Ok(await service.RestoreAsync(repoId, GetUserId(principal), request, ct));
        });

        group.MapPost("/repositories/{id}/stage", async (string id, GitStageRequest request, ClaimsPrincipal principal, RelaxKonOS.Server.Git.IGitRepositoryService service, CancellationToken ct) =>
        {
            if (!Guid.TryParse(id, out var repoId)) return Results.BadRequest();
            return Results.Ok(await service.StageAsync(repoId, GetUserId(principal), request, ct));
        });

        group.MapPost("/repositories/{id}/unstage", async (string id, GitUnstageRequest request, ClaimsPrincipal principal, RelaxKonOS.Server.Git.IGitRepositoryService service, CancellationToken ct) =>
        {
            if (!Guid.TryParse(id, out var repoId)) return Results.BadRequest();
        ...
        // ── 路径探测与初始化（不依赖已注册仓库）──
        group.MapGet("/probe", async (string path, ClaimsPrincipal principal, RelaxKonOS.Server.Git.IGitRepositoryService service, CancellationToken ct) =>
        {
        ...
        group.MapPost("/init", async (string path, ClaimsPrincipal principal, RelaxKonOS.Server.Git.IGitRepositoryService service, CancellationToken ct) =>
        {
        ...
        // ── 远程（remote）管理 ──
        group.MapGet("/repositories/{id}/remotes", async (string id, ClaimsPrincipal principal, RelaxKonOS.Server.Git.IGitRepositoryService service, CancellationToken ct) =>
        {
            if (!Guid.TryParse(id, out var repoId)) return Results.BadRequest();
        ...
        group.MapPost("/repositories/{id}/remotes", async (string id, GitRemoteRequest request, ClaimsPrincipal principal, RelaxKonOS.Server.Git.IGitRepositoryService service, CancellationToken ct) =>
        {
            if (!Guid.TryParse(id, out var repoId)) return Results.BadRequest();
            return Results.Ok(await service.AddRemoteAsync(repoId, GetUserId(principal), request, ct));
        });

        group.MapPut("/repositories/{id}/remotes/{name}", async (string id, string name, GitRemoteRequest request, ClaimsPrincipal principal, RelaxKonOS.Server.Git.IGitRepositoryService service, CancellationToken ct) =>
        {
            if (!Guid.TryParse(id, out var repoId)) return Results.BadRequest();
            return Results.Ok(await service.UpdateRemoteAsync(repoId, GetUserId(principal), name, request, ct));
        });

        group.MapDelete("/repositories/{id}/remotes/{name}", async (string id, string name, ClaimsPrincipal principal, RelaxKonOS.Server.Git.IGitRepositoryService service, CancellationToken ct) =>
        {
            if (!Guid.TryParse(id, out var repoId)) return Results.BadRequest();
