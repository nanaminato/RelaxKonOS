...
对会连接正在运行的 RemoteOS Shell 的命令设置 `REMOTEOS_DEV_TOKEN`（或传入 `--token`）：`--install`、`watch`、`apps`、`install`、`update`、`launch` 和 `uninstall`。POSIX shell 使用 `export REMOTEOS_DEV_TOKEN="<pairing-token>"`，Windows PowerShell 使用 `$env:REMOTEOS_DEV_TOKEN = "<pairing-token>"`。使用 `watch --no-install` 可在不连接 Shell 的情况下构建包。
