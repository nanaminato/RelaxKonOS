...
Linux 的 systemd unit 以及 Windows 上固定 Mihomo 进程的启动、停止与重启均经由受限权限助手。Windows 权限助手以 LocalSystem 启动唯一的、固定路径且由 Server 验证过的 Mihomo 二进制；Server 保持 LocalService 身份。该 Helper 不接受任意文件路径、可执行文件、参数或命令文本。安装、卸载与回滚前后由 Server 记录无秘密操作状态；无法访问受保护目录或 Helper 时安全返回问题码，绝不尝试 sudo 命令回退。
