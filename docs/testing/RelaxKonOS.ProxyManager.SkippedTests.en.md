# RelaxKonOS Proxy Manager — skipped platform tests
...
|---|---|---|---|---|
| PM-G5-WIN-01 | Windows / Windows Server | LocalSystem Helper-managed Mihomo start, stop, restart, update and uninstall | Helper-owned process tree is cleaned up on helper shutdown; protected paths and restart behavior work; no Defender or firewall changes | No isolated, privileged Windows test host or approved Mihomo runtime is available. |
| PM-G5-WIN-02 | Windows / Windows Server | TUN enable, disable and emergency restore | Current RelaxKonOS session, listener, gateway, LAN, SSH/RDP routes and DNS remain reachable; the configured TUN adapter becomes active before success is recorded; recovery restores the original network state | This changes host routing/DNS and must not be first exercised on the development host. |
...
initial verification.  Record the environment, Mihomo artifact hash, RelaxKonOS
revision, result and any problem code beside each case when it is executed.

The in-process coverage in `RelaxKonOS.Server.Tests` exercises the fail-closed
...
|---|---|---|
| PM-G6-G8-API-01 | Run `RelaxKonOS.Server.Tests` after API host fixture coverage for Proxy authorization, idempotency, operation recovery and audit output is provisioned. | This implementation pass is compile-only; no Server process or test suite was started. |
