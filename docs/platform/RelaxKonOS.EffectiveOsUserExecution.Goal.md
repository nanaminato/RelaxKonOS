...
> Linux System Mode 文件领域的后续规则见 [宿主管理员身份与执行路由 Goal](./RelaxKonOS.HostPrivilegeRouting.Goal.md)。本文下方关于“root 文件不可用”和“管理员文件权限不足绝不自动路由”的段落仅保留为原阶段实施记录，已不再是当前文件 API 规范。root 仍不得进入普通 user-execution worker；Terminal、Git 等领域仍按各自已实现规则处理。
