...
- Helper 的普通写入与上传也已改用同目录事务 staging，不再直接截断最终文件。事务清单在写入内容前落盘，记录创建进程 PID/启动时间和最终目标；同目录的后续操作或目录枚举会在目标用户身份下回收已退出进程的事务：提交前中断则恢复旧目标，提交后中断则保留新目标并清除旧备份，存活中的并发 Helper 事务不会被误删。替换已有普通文件时保留其 Unix mode；伪造或损坏、无法验证的相似隐藏目录不会被自动删除。
...
- Linux user-execution 路径的 openat-style TOCTOU hardening，以及真实进程强制终止与 root-owned 残留演练；代码已具备可验证事务的 staging 恢复，sudoers 命令形状、跨 filesystem staged move、运行时取消/超时和安装快照替换也已接入，但仍需在隔离多用户环境验证 SIGKILL、不同用户共享目录及崩溃窗口。
