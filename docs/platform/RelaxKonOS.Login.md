# RelaxKonOS Login 模块设计文档

> 本文档定义 RelaxKonOS 登录模块：客户端登录窗口（mstsc 风格）、传输层、认证会话、服务端 auth 端点、JWT、IIdentityProvider 抽象及登录防护。
>
> 本文档描述**已实现的登录模块**：客户端可真实登录到本机 Server 并进入桌面。不含 SignalR Hub（桌面状态同步是独立模块）。
>
> - 通信契约见 [`RelaxKonOS.Protocol.md`](../architecture/RelaxKonOS.Protocol.md)
> - 身份模型与认证原则见 [`RelaxKonOS.Authentication.md`](./RelaxKonOS.Authentication.md)
> - Workspace / Session / Device 模型见 [`RelaxKonOS.Workspace.md`](../architecture/RelaxKonOS.Workspace.md)
> - 登录成功后的桌面外壳（连接栏、断开 = 登出）见 [`RelaxKonOS.Desktop.md`](../desktop/RelaxKonOS.Desktop.md)
> - 整体架构见 [`RelaxKonOS.Architecture.md`](../architecture/RelaxKonOS.Architecture.md)
...
登录窗口提供显示语言选择器。未认证时，选择写入本机的 `LocalApplicationData/RelaxKonOS/client-language.json`，因此下次启动仍使用该语言；该文件只保存 BCP-47 语言标识，不含凭据或令牌。认证成功后，客户端加载当前 user-workspace 的 `WorkspacePreferencesDto.Language`，并以服务器偏好作为桌面和内置应用的语言来源。注销后恢复本机登录语言。
...
```text
RelaxKonOS.Server.Domain.{User, Workspace, Session, Device}   (领域模型, ToDto() 映射)
    |
RelaxKonOS.Server.Storage.{IUserRepository, IWorkspaceRepository, ISessionRepository, IDeviceRepository}
    |
...
- **桌面外壳衔接**：登录后 `MainWindow` 的 mstsc 连接栏"关闭连接"与标题栏"关闭"均触发 `IAuthSession.LogoutAsync()` 后 `MainWindow.Close()`（当前阶段断开即退出进程，不回登录窗），详见 [`RelaxKonOS.Desktop.md`](../desktop/RelaxKonOS.Desktop.md) §2.4。
...
Linux 服务端部署要求系统提供 PAM 运行库（Ubuntu 的 `libpam0g`，通常由基础系统预装）以及可用的 `login` PAM service。RelaxKonOS 不读取 `/etc/shadow`，也不保存或记录登录请求中的密码；PAM conversation 仅在同步认证调用期间持有凭据。账户信息通过 NSS 查询，因此系统配置的 SSSD/LDAP 用户同样可被解析。

---

## 9. AI Agent 理解规则

实现 RelaxKonOS 登录模块时必须遵守：
...
登录窗口的“计算机”输入框同时是已保存连接的下拉列表。RelaxKonOS 按 **Server URL + 用户名** 保存多条独立记录；因此同一台服务器可使用不同账户，多台服务器也不会互相覆盖。
...
选择 SSH 登录方式时，同一套“计算机”下拉框改为展示已保存的 SSH 记录：本机宿主记录（SSH 地址、端口、用户名）与安全存储中的 SSH 凭据按「主机:端口 + 用户名」合并为「用户名@主机:端口」项，地址栏只写入 `主机:端口`。在 SSH 方式下**选中的记录同时拥有地址与用户名**：可编辑下拉框把地址文本回写成选择结果，因此切换登录方式后，记录的用户名必须优先于上一次 SSH 会话缓存下来的输入，否则会出现“已回填密码但用户名为空”的状态；地址栏未命中任何记录且用户尚未输入用户名时，自动选中最近使用的记录。用户自己输入的地址与用户名在没有记录认领该地址时跨登录方式切换保留。
