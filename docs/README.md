...
RelaxKonOS 采用状态同步模式（非像素流）：Client 本地渲染 UI，与 Server 同步状态/数据/命令。
...
Framework/
    RelaxKonOS.Core                平台无关原语与类型
    RelaxKonOS.UI                  Avalonia 共享主题/样式
    RelaxKonOS.WindowManager       窗口管理器 + RemoteWindow 控件
    RelaxKonOS.App.SDK             应用开发面（AppContext / IRemoteApplication）
    RelaxKonOS.Runtime             应用运行时（ApplicationManager）
Shared/
    RelaxKonOS.Protocol            通信协议契约（Common/Identity/Workspace/Desktop/Files/Hubs，已完整定义）
RelaxKonOS.Server/                 服务端（ASP.NET Core，跨平台，已实现 auth 端点）
RelaxKonOS.Guardian.Agent/         进程守护独立进程（原生服务管理，命名管道 IPC）
...
### 4.1 RelaxKonOS.Client.Desktop

- **类型**：Executable (WinExe)
- **定位**：RelaxKonOS 平台启动入口，类似 Windows Boot Loader / Desktop Entry。
- **职责**：Avalonia AppBuilder、平台初始化、字体配置、日志配置、启动 `RelaxKonOS.Client`。
- **不包含**：Shell 逻辑、应用逻辑、窗口逻辑。

### 4.2 RelaxKonOS.Client

- **类型**：Class Library
- **定位**：RelaxKonOS Shell，类似 `explorer.exe`。
...
- **职责**：创建窗口、关闭窗口、移动、Resize、Focus、Minimize、Maximize、Z Order、Taskbar State、**模态对话框（`ShowDialogAsync` + `ModalDialog<TResult>` + owner 局部遮罩）**。详见 [`RelaxKonOS.Desktop.md`](./desktop/RelaxKonOS.Desktop.md) §3。
- **窗口创建流程**：
...
### 4.6 RelaxKonOS.App.SDK

- **定位**：RelaxKonOS 应用开发接口，类似 Windows SDK / Android SDK。
- **提供**：
  - **Window API**（已实现）：`AppContext.ShowWindow()`
  - **Modal Dialog API**（已实现）：`AppContext.ShowDialogAsync<TResult>(owner, title, contentFactory)` — 可复用、可嵌套、任意结果类型，详见 [`RelaxKonOS.Desktop.md`](./desktop/RelaxKonOS.Desktop.md) §3
...
### 4.7 RelaxKonOS.Runtime

- **定位**：应用运行时。RelaxKonOS Application 不是普通 exe。
...
- **包含**：DTO（sealed record + `[property: JsonPropertyName]`）、API Contract（`*ApiRoutes` 路由常量）、SignalR Hub 接口（`IWorkspaceHubClient` / `ITerminalHubClient` / `IPerformanceHubClient` / `IGuardianLogsHubClient` + Methods/Events 常量）、序列化约定。Client Proxy 实现位于 `RelaxKonOS.Client`，Hub/端点实现位于 `RelaxKonOS.Server`。详见 [`RelaxKonOS.Protocol.md`](./architecture/RelaxKonOS.Protocol.md)。
- **规则**：所有 Client / Server 通信必须经过 Protocol。禁止业务代码直接调用 HTTP / WebSocket。Protocol 程序集零 PackageReference。

### 4.9 RelaxKonOS.Server

- **定位**：RelaxKonOS Cloud Backend，**跨平台运行于 Ubuntu / Windows Server**。已实现 auth 端点（login/refresh/logout/me）+ JWT + `IIdentityProvider`（Windows LogonUser / Linux PAM + NSS）+ 持久化仓储（EF Core + SQLite + HostGlobal 自写迁移器，User/Workspace(含 TerminalSettings/BrowserSettings/Preferences)/Device/Bookmark/HistoryEntry/AppSettings/ImageMirrors/GitRepository/Tunnel*/Registry*/AuthenticationProtection 落业务库，Certificate*/WebServer* 等宿主级资源落 HostGlobal 版本化迁移）+ 文件管理端点（`/api/v1.0/files/*`）+ 浏览器端点（`/api/v1.0/browser/*`，按用户隔离书签/历史 + `BrowserSettings` 持久化）+ Workspace 端点（preferences / window-layout）+ 系统监控端点（`/api/v1.0/system/*` + SignalR `/hubs/performance` 1Hz 推送）+ 应用能力端点（`/api/v1.0/capabilities`）+ AppSettings 端点（`/api/v1.0/app-settings`，应用私有 KV）+ 注册表端点（`/api/v1.0/registry`，配置注册表 desired/applied 状态机）+ 镜像源端点（`/api/v1.0/image-mirrors`，APT/Docker/NPM 等镜像源配置）+ Docker 管理端点（`/api/v1.0/docker/*`：status/containers/images/stacks/networks/volumes，`IDockerEngineService` + `IDockerComposeService` 调 `docker` CLI）+ 进程守护端点（`/api/v1.0/guardian/*` + SignalR Hub `/hubs/guardian-logs`，`IProcessGuardianService` 通过命名管道与 Guardian Agent IPC）+ WebServer 端点（`/api/v1.0/webservers/*`：实例/站点/快照/操作）+ 证书端点（`/api/v1.0/certificates/*`：ACME 申请、续期、部署、吊销，HostGlobal 表持久化）+ Git 端点（`/api/v1.0/git/*`：仓库/分支/提交/拉取/推送/冲突/历史）+ 隧道端点（`/api/v1.0/tunnels/*`：FRP Server Profile / Definition / Secrets / Audit）+ 防火墙端点（Linux UFW，`/api/v1.0/firewall/*`）+ 健康检查端点（`/healthz` / `/ready`）。详见各应用文档与 [`RelaxKonOS.Storage.md`](./platform/RelaxKonOS.Storage.md)、[`RelaxKonOS.Protocol.md`](./architecture/RelaxKonOS.Protocol.md)。
...
- **不包含**：生产代码。验证通过的能力后续迁移到 `RelaxKonOS.Server` 的 OS 抽象层实现中。

### 4.11 RelaxKonOS.Guardian.Agent
...
- **详见**：[`RelaxKonOS.ProcessGuardian.md`](./applications/RelaxKonOS.ProcessGuardian.md)

---

## 5. Application 开发模型

RelaxKonOS Application 结构：
...
- **已实现**：导航（后退/前进/刷新/停止/主页/地址栏）+ 书签（加入/删除/侧边栏双击导航/清空全部）+ 历史（自动记录访问/侧边栏双击导航/单条删除/清空全部）+ 浏览器偏好持久化；JWT via IAuthSession；未登录弹提示窗。详见 [`RelaxKonOS.Browser.md`](./applications/RelaxKonOS.Browser.md)。

### Port Forwarding

- **定位**：Client 本机 SSH 隧道管理器；可由浏览器或第一方服务请求，不经 Server 同步配置。
- **已实现**：优先绑定请求端口、冲突时自动选取可用 loopback 端口、返回实际链接、运行中隧道列表、更新与停止。详见 [`RelaxKonOS.PortForwarding.md`](./applications/RelaxKonOS.PortForwarding.md)。
...
- **已实现**：性能页（CPU 整机+每核+柱状图 / 内存柱状图 / 磁盘 / 网络速率 / GPU nvidia-smi / 运行时间，2s 自动刷新）+ 进程页（列表 + 按名称/PID/用户过滤 + 结束任务，权限不足提示需在宿主 OS 提权）；JWT via IAuthSession；未登录弹提示窗。详见 [`RelaxKonOS.TaskManager.md`](./applications/RelaxKonOS.TaskManager.md)。
...
- **已实现**：UI 移植自 Jaya File Manager（BSD-3），导航树（懒加载）+ Explorer 网格 + 地址栏 + 工具栏 + 状态栏。所有文件操作经 Server 端 REST API（`/api/v1.0/files/*`）执行，Server 以宿主 OS 进程身份执行 `System.IO`，复用宿主用户/权限（不另建 ACL——project_memory 硬约束）。支持浏览、新建文件夹/删除/重命名/复制/移动/上传/下载、文件/目录属性查看（Linux 可编辑 POSIX 权限），以及依据应用 manifest 扩展名声明的默认打开与“打开方式”；危险操作（删除）弹确认对话框。JWT 复用 `IAuthSession`。详见 [`RelaxKonOS.Explorer.md`](./applications/RelaxKonOS.Explorer.md)。
...
| 阶段 | 内容 | 状态 |
|------|------|------|
| 阶段 0 | Desktop / Wallpaper / Icon / Taskbar / WindowManager | 完成（+ 宿主窗口控制 / mstsc 连接栏 / 模态对话框，见 [`RelaxKonOS.Desktop.md`](./desktop/RelaxKonOS.Desktop.md)） |
...
| 阶段 3 | RemoteServer：Account / Workspace / Sync / Storage / Remote State | 完成（登录模块；服务端 SQLite 双域持久化——业务库 User/Workspace/Device/Bookmark/HistoryEntry/AppSettings/ImageMirrors/Git/Tunnel*/Registry*/AuthenticationProtection + HostGlobal 证书/WebServer v1~v7 版本化迁移；设置中心——偏好扩展到主题调色板/桌面显示/文本编码/窗口布局 + 多设备同步；见 [`RelaxKonOS.Storage.md`](./platform/RelaxKonOS.Storage.md) / [`RelaxKonOS.Settings.md`](./desktop/RelaxKonOS.Settings.md)） |
...
1. `RelaxKonOS.Client`
2. `RelaxKonOS.WindowManager`
3. `RelaxKonOS.Core`
4. `RelaxKonOS.Runtime`
5. `RelaxKonOS.App.SDK`
...
- **登录与身份**（Windows LogonUser、Linux PAM + NSS 均已实现）— 已完成，见 [`RelaxKonOS.Login.md`](./platform/RelaxKonOS.Login.md)；设计原则见 [`RelaxKonOS.Authentication.md`](./platform/RelaxKonOS.Authentication.md)
- **安全设计**（sudo / 权限 / 危险操作确认）— 见 [`RelaxKonOS.Security.md`](./platform/RelaxKonOS.Security.md)
- **Workspace / Session / Device 多设备模型** — 见 [`RelaxKonOS.Workspace.md`](./architecture/RelaxKonOS.Workspace.md)
- **云同步、Storage、Remote Runtime、Compute** — 见 [`RelaxKonOS.Architecture.md`](./architecture/RelaxKonOS.Architecture.md)
...
| 关注点 | 归属 |
|--------|------|
| 窗口逻辑 | `RelaxKonOS.WindowManager` |
| 应用生命周期 | `RelaxKonOS.Runtime` |
| 系统入口 / Shell | `RelaxKonOS.Client` |
| 网络通信 | `RelaxKonOS.Protocol` |

---

## 12. AI Agent 快速理解

修改 RelaxKonOS 代码前必须理解：

```text
RelaxKonOS            = Operating System Shell
RelaxKonOS.Client     = Desktop Shell
RelaxKonOS.WindowManager = Window System
RelaxKonOS.Runtime    = Application Runtime
RelaxKonOS.Server     = Cloud Backend
```

- **不要**将 RelaxKonOS 实现为 Remote Desktop Tool 或 Web Management Dashboard。
...
| 文档 | 用途 |
|------|------|
| [`Authentication`](./platform/RelaxKonOS.Authentication.md) | 登录系统、Linux 用户集成、身份模型 |
| [`Authentication Hardening`](./platform/RelaxKonOS.Authentication.Hardening.md) | 认证限流、风险控制与登录防护建议 |
...
| 文档 | 用途 |
|------|------|
| [`Browser`](./applications/RelaxKonOS.Browser.md) | 浏览器、书签与历史 |
| [`CodeEditor`](./applications/RelaxKonOS.CodeEditor.md) | 代码编辑器与文件安全边界 |
| [`CertificateManager`](./applications/RelaxKonOS.CertificateManager.md) | ACME 证书生命周期、Kestrel 部署与续期 |
| [`DockerManager`](./applications/RelaxKonOS.DockerManager.md) | Docker Engine、容器与 Stack 管理 |
| [`Explorer`](./applications/RelaxKonOS.Explorer.md) | 文件管理器、REST API 与权限复用 |
| [`Firewall`](./applications/RelaxKonOS.Firewall.md) | Linux Server UFW 防火墙 |
| [`GitClient`](./applications/RelaxKonOS.GitClient.md) | Git 仓库版本控制、分支与提交 |
...
| [`Develop`](./development/RelaxKonOS.Develop.md) | 开发者快速上手、代码结构与调试指南 |
| [`DeveloperMode`](./development/RelaxKonOS.DeveloperMode.md) | 开发模式、DevCli 与应用包发布 |
