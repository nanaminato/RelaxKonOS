...
>
> - Windows 11 体验优化的当前完成项、验证与后续 API 计划见 [`RelaxKonOS.Explorer.Progress.md`](./RelaxKonOS.Explorer.Progress.md)（2026-09-06）
> - 架构原则见 [`RelaxKonOS.Architecture.md`](../architecture/RelaxKonOS.Architecture.md)
> - 项目当前状态见 [`RelaxKonOS.md`](../README.md)（§6 内置应用 / §7 RemoteExplorer）
> - 桌面外壳与窗口管理见 [`RelaxKonOS.Desktop.md`](../desktop/RelaxKonOS.Desktop.md)
> - 登录与身份认证见 [`RelaxKonOS.Login.md`](../platform/RelaxKonOS.Login.md)（Explorer 复用 `IAuthSession` JWT）
> - 通信协议契约见 [`RelaxKonOS.Protocol.md`](../architecture/RelaxKonOS.Protocol.md)（§Files DTO 与路由）

---

## 1. 定位

RemoteExplorer 是 RelaxKonOS 的内置文件管理器，用于浏览与操作**服务端宿主 OS** 的文件系统。

- **架构归属**：§6.2 Remote Service Application —— UI 在 Client 本地渲染，文件 IO 在 Server 端执行。
- **复用宿主 OS 权限**（project_memory 硬约束）：Server 以宿主 OS 进程身份执行 `System.IO`，复用宿主用户/权限，**不另建** RelaxKonOS ACL 系统。权限提升（sudo/UAC）后续接入（见 §7 后续）。
...
```text
┌───────────── Client (RelaxKonOS.Client/Apps/Explorer) ─────────────┐    ┌──────── Server (RelaxKonOS.Server/Files) ────────┐
│                                                                  │    │                                                │
...
│    └─ ExplorerClient                                             │    │                                                │
└──────────────────────────────────────────────────────────────────┘    └────────────────────────────────────────────────┘
              ↕  REST /api/v1.0/files/*  +  Shared/RelaxKonOS.Protocol/Files (DTO + FileApiRoutes)
```
...
[`RelaxKonOS.Server/Files/IFileService.cs`](../../RelaxKonOS.Server/Files/IFileService.cs) 定义接口；[`LocalFileService.cs`](../../RelaxKonOS.Server/Files/LocalFileService.cs) 实现。
...
路由常量见 [`FileApiRoutes`](../../Shared/RelaxKonOS.Protocol/Files/FileApiRoutes.cs)，均 `$"/api/v1.0/files/..."`。
...
```
Client/RelaxKonOS.Client/Apps/Explorer/
...
合并 Jaya 的 5 个 VM（`ExplorerViewModel` / `NavigationViewModel` / `AddressbarViewModel` / `ToolbarViewModel` / `StatusbarViewModel`）为单一 `ExplorerViewModel`，适配 RelaxKonOS DI 约定（避免引入 Jaya 的 `ServiceLocator` / `EventAggregator` / `ViewModelLocator` 反射基础设施）。
...
services.AddSingleton<IRemoteApplication, RelaxKonOS.Client.Apps.Explorer.ExplorerApp>();
```
...
10. **编译验证**：`dotnet build RelaxKonOS.sln -c Debug` 必须 0 错误（NU1903 Microsoft.OpenApi 与 CS0169 TerminalSession._disposed 为既有警告，非本模块引入）。
