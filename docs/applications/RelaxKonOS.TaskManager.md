# RelaxKonOS TaskManager 模块设计

> **迁移状态（2026-08-24）**：本文以下内容描述第一代 REST 轮询实现。当前架构见 [`RelaxKonOS.TaskManager.Rewrite.md`](./RelaxKonOS.TaskManager.Rewrite.md)：性能页使用“Server 统一采样 + 内存历史 + SignalR 推送”，进程页使用独立低频采样与分页查询。旧 `/api/v1.0/system/metrics` 与非分页 `/processes` 已删除。
...
> - 项目当前状态见 [`RelaxKonOS.md`](../README.md)（§6 内置应用）
> - 桌面外壳与窗口管理见 [`RelaxKonOS.Desktop.md`](../desktop/RelaxKonOS.Desktop.md)
> - 登录与身份认证见 [`RelaxKonOS.Login.md`](../platform/RelaxKonOS.Login.md)（TaskManager 复用 `IAuthSession` JWT）
> - 通信协议契约见 [`RelaxKonOS.Protocol.md`](../architecture/RelaxKonOS.Protocol.md)（§SystemMonitor DTO 与路由）
> - 服务端持久化见 [`RelaxKonOS.Storage.md`](../platform/RelaxKonOS.Storage.md)（TaskManager 不落库——指标与进程列表均为实时采样）
> - 安全设计见 [`RelaxKonOS.Security.md`](../platform/RelaxKonOS.Security.md)（§权限提升委托宿主 OS——结束进程不自动提权）

---

## 1. 定位

TaskManager 是 RelaxKonOS 的内置系统监控应用，参考 Windows 任务管理器 / GNOME 系统监视器。

- **架构归属**：§6.2 Remote Service Application —— UI 完全在 Client 本地渲染；指标与进程列表**真源在 Server 端实时采集**（不持久化，每次请求都是当下快照）。
- **复用宿主 OS 权限**（硬约束）：Server 端 `ISystemMetricsProvider` 以宿主 OS 进程身份读取 `/proc`（Linux）或 Win32 API（Windows），复用宿主用户/权限，不另建 ACL。结束进程权限不足时返回 `RequiresElevation=true`，**RelaxKonOS 不自动提权**——用户需在宿主 OS 提权（`sudo kill` / UAC 运行）。
...
## 3. 协议契约（`Shared/RelaxKonOS.Protocol/SystemMonitor/`）
...
### 4.1 跨平台抽象（`RelaxKonOS.Server.SystemMonitor/`）
...
**权限不足错误码**：Windows `ERROR_ACCESS_DENIED=5`；Linux `EPERM=1` / `EACCES=13`——映射为 `RequiresElevation=true`。符合硬约束「权限提升委托宿主 OS」——RelaxKonOS 不存储宿主密码、不自动提权。

### 4.5 REST 端点（`RelaxKonOS.Server.Endpoints/SystemMonitorEndpoints.cs`）
