...
内置 ID 改为稳定命名空间：`remoteos.default`、`remoteos.windows-like`、`remoteos.macos-like`、`remoteos.ubuntu-like`。读取旧值 `remoteos`、`windows-like`、`macos-like`、`ubuntu-like` 时在客户端归一化到新 ID；服务端在迁移期同时接受旧值，写回时只写新值。
...
在上述任一步失败时：移除候选视图、释放候选、重新挂回旧 surface；如果旧 surface 已不可用则启动 `remoteos.default`。切换期间应显示宿主级不可交互遮罩，且不得关闭、最小化或重建现有 `ManagedWindow`。
...
- 切换中的外部 Shell 崩溃必须保留现有应用窗口；运行时立即回退到 `remoteos.default` 并向用户显示可复制的诊断 ID。
...
2. 包不存在、版本/API 不兼容或被禁用时，激活 `remoteos.default`，但不覆盖服务端选择。
