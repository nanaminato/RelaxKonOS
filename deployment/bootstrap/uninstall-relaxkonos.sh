...
# ownership marker, so an arbitrary directory at this path is preserved rather than deleted.
if [[ -d "$INSTALL_ROOT/versions" || -L "$INSTALL_ROOT/current" || -f "$INSTALL_ROOT/server/RelaxKonOS.Server" ]]; then
  rm -rf -- "$INSTALL_ROOT"
